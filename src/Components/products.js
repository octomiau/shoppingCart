      ...
      return (
        <Accordion.Item key={1+index} eventKey={1 + index}>
          ...
          <Accordion.Body onClick={() => deleteCartItem(index)}
            style={{ cursor: "pointer" }}eventKey={1 + index}>
            $ {item.cost} from {item.country}. Click to delete item.
          </Accordion.Body>
        </Accordion.Item>
      );
    });
    ...
    return (
      <Container>
        <Row>
          ...
          <Col>
            <h1>CheckOut </h1>
            <Button variant="btn btn-outline-success" onClick={checkOut}>CheckOut $ {finalList().total}</Button>
